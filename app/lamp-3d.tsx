...
export default function Lamp3D({motion,replay,onReplay}:{motion:boolean;replay:number;onReplay:()=>void}){
 ...
 const scene=new T.Scene(),camera=new T.OrthographicCamera(-7,7,4.2,-4.2,.1,80);camera.position.set(5,6.8,25);camera.lookAt(0,3.3,0);
 ...
 const letters=[letter('M',3.5,-3.55,ivory),letter('&',2.7,0,lime),letter('M',3.5,3.55,ivory)];
 ...
 const points=[[-4.65,3.5],[-2.4,3.5],[-.4,2.7],[2.4,3.5],[4.65,3.5]],flights=[[.7,1.7],[2.15,3.3],[3.75,4.85],[5.35,6.85]];let renderedTime=-1,t=0,last=performance.now(),seen=controls.current.replay,visible=true;
 const observer=new IntersectionObserver(([e])=>{visible=e.isIntersecting;});observer.observe(el);
 let dirty=true;function resize(){dirty=true;const w=el.clientWidth,h=el.clientHeight;renderer.setSize(w,h);const ratio=w/h,halfW=Math.max(6.8,4.1*ratio);camera.left=-halfW;camera.right=halfW;camera.top=halfW/ratio;camera.bottom=-halfW/ratio;camera.updateProjectionMatrix();}const ro=new ResizeObserver(resize);ro.observe(el);resize();
 ...
 for(let i=0;i<4;i++){const[a,b]=flights[i],p=points[i],q=points[i+1];if(t>=b){x=q[0];y=q[1];const age=t-b;const compression=age<.55?Math.sin(Math.min(age/.12,1)*Math.PI/2)*Math.exp(-age*6)*(.19)*(age<.2?1:Math.cos((age-.2)*18)):0;const letterIndex=i===0?0:i===1?1:2;letters[letterIndex].scale.y=1-compression;letters[letterIndex].scale.x=1+compression*.14;y*=1-compression;squash=compression*1.4;}
 ...
 lamp.position.set(x,y+.02,0);flip.rotation.z=spin;body.scale.y=1-squash;body.rotation.z=tilt;
 ...
 return <div className="three-stage" ref={host} role="group" aria-label="Объёмные буквы M&M и металлическая прыгающая лампа">{!ready&&<div className="three-fallback">M<span>&amp;</span>M</div>}<button className="three-replay" onClick={onReplay}>Нажмите на лампу ↻</button></div>;
}
